...
Public Class POP_UP_LCN_AUDIT_IN_UPLOAD
    ...
    Public Sub BindTable()
        ...
        dao_f.GetDataby_FK_IDA_AND_TR_ID_AND_TYPE(_IDA, tr_id, 1)
    ...
    Protected Sub btn_add_upload_Click(sender As Object, e As EventArgs) Handles btn_add_upload.Click
        ...
        Else
            Blind_PDF()
            alert("แนบไฟล์เรียบร้อย กรุณากดดูข้อมูลเพื่อตรวจสอบความถูกต้อง")
        ...
        For Each tr As TableRow In tb_type_menu.Rows
            ...
            If f.HasFile Then
                ...
                If exten.ToUpper = "PDF" Then
                    ...
                    Dim Name_fake As String = "ADI-" & _ProcessID & "-" & Date.Now.Year & "-" & IDA & ".pdf"
                    dao_f.fields.NAME_FAKE = Name_fake
                    dao_f.fields.NAME_REAL = f.FileName
                    dao_f.fields.CREATE_DATE = Date.Now
                    dao_f.fields.Status_file = 1
                    dao_f.update()

                    dao.fields.STATUS_FILE_UPLOAD = 1
                    dao.update()
