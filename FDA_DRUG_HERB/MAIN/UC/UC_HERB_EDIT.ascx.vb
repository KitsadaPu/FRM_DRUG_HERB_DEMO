Public Class UC_HERB_EDIT
    ...
    Sub Set_Label(ByVal CITIZEN_ID_AUTHORIZE As String)
        ...
        dao_dal.GetDataby_FK_IDA(Request.QueryString("ida"))
        txt_da_opentime.Text = dao_dal.fields.opentime

            Dim dao_frgn As New DAO_DRUG.TB_DALCN_FRGN_DATA_FIX
        dao_frgn.GetDataby_IDA_DALCN(Request.QueryString("ida"))
        If dao_frgn.fields.PERSONAL_TYPE_MENU = 1 Then
            ...
            If dao_frgn.fields.addr_status = 1 Then 'set ที่อยู่ที่ติดต่อ
                Dim dao_curent As New DAO_DRUG.TB_DALCN_CURRENT_ADDRESS_FIX
            dao_curent.GetDataby_IDA_DALCN(Request.QueryString("ida"))
    ...
    End Function
    Sub setdata(ByRef dao As DAO_DRUG.ClsDBdalcn_fix, ByVal TR_ID As Integer)
        With dao.fields
    ...
    End Sub
    Sub setdata_frgn_data(ByRef dao As DAO_DRUG.TB_DALCN_FRGN_DATA_FIX) 'set บุคคลต่างด้าว
        With dao.fields
            Try
    ...
    Sub set_date_current_addr(ByRef dao As DAO_DRUG.TB_DALCN_CURRENT_ADDRESS_FIX) 'insert ที่อยู่ติดต่อได้
