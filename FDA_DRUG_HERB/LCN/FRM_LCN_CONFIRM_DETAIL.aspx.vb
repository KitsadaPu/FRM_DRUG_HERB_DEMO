...
Public Class FRM_LCN_CONFIRM_DETAIL
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Else
            ...
            End Try
            Dim tr_id As String = ""
            tr_id = "DA-" & _PROCESS_ID & "-" & years & "-" & _TR_ID

            Dim LCNNO_V2 As Integer
            Dim bao2 As New BAO.GenNumber
            Dim RCVNO_HERB_NEW As Integer
            RCVNO = bao.GEN_RCVNO_NO(con_year(Date.Now.Year()), _CLS.PVCODE, _PROCESS_ID, _IDA)
            dao.fields.rcvno = RCVNO 'bao.FORMAT_NUMBER_FULL(con_year(Date.Now.Year()), RCVNO)
            RCVNO_HERB_NEW = bao2.GEN_RCVNO_NO_NEW(con_year(Date.Now.Year()), _CLS.PVCODE, _PROCESS_ID, _IDA)
            ...
            dao.fields.RCVNO_NEW = "HB " & _CLS.PVCODE & "-" & _PROCESS_ID & "-" & con_year(Date.Now.Year).Substring(2, 2) & "-" & RCVNO_HERB_NEW
            ...
            cls_sop.BLOCK_SOP(_CLS.CITIZEN_ID, _PROCESS_ID, "2", "ยื่นคำขอ", tr_id, b64)
            cls_sop.BLOCK_STAFF(_CLS.CITIZEN_ID, "USER", _PROCESS_ID, _CLS.PVCODE, 2, "ส่งเรื่องและรอพิจารณา", "SOP-DRUG-10-" & _PROCESS_ID & "-1", "รับคำขอ", "รอเจ้าหน้าที่รับคำขอ", "STAFF", tr_id, SOP_STATUS:="ยื่นคำขอ")

            AddLogStatus(Status_ID, _PROCESS_ID, _CLS.CITIZEN_ID, _IDA)
