...
Public Class FRM_LCN_AUDIT_OUT
    ...
    Function bind_data()
        ...
        End If
        dt = bao.SP_DALCN_AUDIT_OUT(IDEN, _IDA_LCN)
        Return dt
    End Function
        ...
        If TypeOf e.Item Is GridDataItem Then
            ...
            If e.CommandName = "HL_SELECT" Then
                If STATUS_ID = 11 Then
                    ...
                    System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "Popups2('" & " POP_UP_LCN_AUDIT_OUT_ERQUEST.aspx?IDA=" & IDA & "&TR_ID=" & TR_ID & "&PROCESS_ID=" & _PROCESS_ID & "&IDA_LCN=" & _IDA_LCN & "');", True)
                Else
                    System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "Popups2('" & " POP_UP_LCN_AUDIT_OUT_CONFIRM.aspx?IDA=" & IDA & "&PROCESS_ID=" & _PROCESS_ID & "');", True)
                End If
                'If STATUS_ID <> 0 Then
                '    lbl_head1.Text = "แก้ไขข้อมูลและอัพโหลเอกสาร"
                '    System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "Popups2('" & " POP_UP_LCN_AUDIT_OUT_CONFIRM.aspx?IDA=" & IDA & "&PROCESS_ID=" & _PROCESS_ID & "');", True)
    ...
    Protected Sub btn_add_Click(sender As Object, e As EventArgs) Handles btn_add.Click
        Try
            ...
            If IDEN = "" Then
                IDEN = _CLS.CITIZEN_ID_AUTHORIZE
            End If
            System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "Popups2('" & "POP_UP_LCN_AUDIT_OUT_ADD.aspx?IDA_LCN=" & _IDA_LCN & "&IDENTIFY=" & IDEN & "&PROCESS_ID=" & _PROCESS_ID & "');", True)
        Catch ex As Exception
