...
Imports System.IO
Public Class UC_DR_TRANFER_UPLOAD
            ...
            ElseIf e.CommandName = "tranfer" Then
                Try
                    If RadioButtonList1.SelectedValue <> "" Then
                        ...
                        dao.GetDataby_IDA(IDA)
                        'Panel1.Style.Add("display", "block")
                        System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "Popups2('../DR/POPUP_DR_UPLOAD.aspx?IDA=" & _main_ida & "&IDA_TF=" & IDA & "&process=" & _processID & "&lcn_ida=" & _lcn_ida & "');", True)
                        'Response.Redirect(HttpContext.Current.Request.Url.AbsoluteUri & "&IDA_TF=" & IDA)
                    Else
                        System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "alert('กรุณาเลือกประเภท');", True)
                    End If
